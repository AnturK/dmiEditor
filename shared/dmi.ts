...
export class Dmi {
    ...
    buildData() {
        ...
        const result_image = DmiState.empty_frame(png_width, png_height) as unknown as ImageDurr &
            Image;
    ...
    async getFileData() {
        ...
        const result = await encodePng(data, {
            ...
            ancillaryChunksAfterIHDR: true
        });
        return result.data;
    }

    serialize() {
        const obj: SerializedDmi = {
            width: this.width,
            height: this.height,
    ...
    static async parse(data: Uint8Array) {
        ...
        if (lines.next().value !== "# BEGIN DMI") throw Error("Missing metadata header.");
        ...
        for (const line of lines) {
            ...
            switch (key) {
                ...
                case "delay":
                    ...
                    state.delays = value
                        .split(",")
                        .map(x => (x.includes(".") ? parseFloat(x) : parseInt(x)));
                    break;
                case "loop":
                    if (state === undefined)
                        throw Error("Out of order state informaton in metadata.");
...
function resizeArray<T>(oldArray: Array<T>, new_size: number, fillFunc: (index: number) => T) {
    return [...Array(new_size)].map((_, index) => {
        if (oldArray.length > index) return oldArray[index];
        else return fillFunc(index);
    });
}
