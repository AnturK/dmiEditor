...
const Editor: React.FC = () => {
	...
	const saveZoom = (value: number) => { select.sessionPersistentData.setZoom(value); setZoom(value); };
	...
	useEffect(() => {
		...
		const fetchDmi = async () => {
			const initial_dmi_data = await select.messageHandler.sendRequest<ReadyResponseMessage>({ type: MessageType.ReadyRequest });
	...
	const pushDmiUpdate = (new_dmi: Dmi) => {
		select.messageHandler.sendEvent({ type: MessageType.EditEvent, edited_dmi: new_dmi.serialize() });
		setDmi(new_dmi);
	};
	...
	if(openStateIndex != null){
		mainContent = <StateDetailView state={dmi.states[openStateIndex]} pushUpdate={state => pushStateUpdate(state,openStateIndex)}/>;
		//Close state view button
		infoBarElements.push(<VSCodeButton onClick={() => setOpenStateIndex(null)}>Close</VSCodeButton>);
	}
	else{
		mainContent = <StateList filterString={searchText} dmi={dmi} pushUpdate={pushDmiUpdate} onOpen={state => setOpenStateIndex(dmi.states.findIndex(x => x === state))} />;
		//Search bar
		infoBarElements.push(<VSCodeTextField value={searchText} onInput={e => setSearchText((e.target as HTMLInputElement).value)}><span slot="start" className="codicon codicon-search"></span></VSCodeTextField>);
	}
