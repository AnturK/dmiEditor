...
export const StateDetailView: React.FC<StateDetailViewProps> = props => {
    ...
    const previewGrid = () => {
        switch (props.state.dirs) {
            case 1:
                return (
                    <div className="singleDirGrid">
                        <img className="frame" src={state.generate_preview(Dirs.SOUTH)} />
                    </div>
                );
            case 4:
                return (
                    <div className="dirGrid">
                        <div></div>
                        <div>
                            <img className="frame" src={state.generate_preview(Dirs.NORTH)} />
                        </div>
                        <div></div>
                        <div>
                            <img className="frame" src={state.generate_preview(Dirs.WEST)} />
                        </div>
                        <div></div>
                        <div>
                            <img className="frame" src={state.generate_preview(Dirs.EAST)} />
                        </div>
                        <div></div>
                        <div>
                            <img className="frame" src={state.generate_preview(Dirs.SOUTH)} />
            ...
            case 8:
                return (
                    <div className="dirGrid">
                        <div>
                            <img className="frame" src={state.generate_preview(Dirs.NORTHWEST)} />
                        </div>
                        <div>
                            <img className="frame" src={state.generate_preview(Dirs.NORTH)} />
                        </div>
                        <div>
                            <img className="frame" src={state.generate_preview(Dirs.NORTHEAST)} />
                        </div>
                        <div>
                            <img className="frame" src={state.generate_preview(Dirs.WEST)} />
                        </div>
                        ...
                        <div>
                            <img className="frame" src={state.generate_preview(Dirs.SOUTHWEST)} />
                        </div>
                        <div>
                            <img className="frame" src={state.generate_preview(Dirs.SOUTH)} />
                        </div>
                        <div>
                            <img className="frame" src={state.generate_preview(Dirs.SOUTHEAST)} />
                        </div>
                    </div>
                );
        }
    ...
    const filledDelays: number[] = state.delays.length
        ? state.delays
        : Array(state.framecount).fill(1);

    const updateName = (new_name: string) => {
        if (state.name == new_name) return;
    ...
    const updateDelay = (delay_index: number, new_value: string) => {
        ...
        if (isNaN(value_as_number)) return;
        const updatedDelays = filledDelays.map((value, index) =>
            index == delay_index ? Math.abs(parseFloat(new_value)) : value
        );
    ...
    const change_dir_count = (e: React.FormEvent<HTMLElement> | Event) => {
        ...
        if (isNaN(new_dircount)) return;
        if (new_dircount == state.dirs) return;
        const new_state = state.clone();
    ...
    const updateLoopCount = (new_count: string) => {
        ...
        if (state.loop == parsedCount) return;
        const new_state = state.clone();
    ...
    const updateFrameCount = (new_count: string) => {
        ...
        if (isNaN(parsedCount)) return;
        if (parsedCount <= 0) return;
        if (state.framecount == parsedCount) return;
        const new_state = state.clone();
    ...
    useGlobalHandler<ClipboardEvent>(
        ...
        async e => {
            ...
            for (let index = 0; index < data.files.length; index++) {
                ...
                if (
                    prospectiveState.width == state.width &&
                    prospectiveState.height == state.height
                ) {
                    ...
                    raw_clipboard_errors.push(
                        `Size of pasted image (${prospectiveState.width}x${prospectiveState.height}) does not match size of DMI (${state.width}x${state.height})`
                    );
                }
            }
                    ...
                    try {
                        ...
                        if (dmi_or_png.width == state.width && dmi_or_png.height == state.height) {
                            ...
                            messageHandler.sendEvent({
                                ...
                                text: `Size of pasted raw image (${dmi_or_png.width}x${dmi_or_png.height}) does not match size of DMI (${state.width}x${state.height})`
                            });
                        }
                    } catch (error) {
        ...
        <div onClick={() => selectedFrame !== null && setSelectedFrame(null)}>
            ...
            <div style={dynamicStyle}>
                <div className="frameGrid">
                    ...
                    {filledDelays.map((delay, index) => (
                        <div className="gridHeader">
                            <EditableField
                                value={delay.toString()}
                                onChange={val => updateDelay(index, val)}
                            />
                        </div>
                    ))}
                    {[...Array(state.dirs)].map((_, dir_index) => (
                        <>
                            ...
                            {[...Array(state.framecount)].map((_, frame) => {
                                const index = state.frame_index(
                                    frame,
                                    DmiState.DIR_ORDER[dir_index]
                                );
                                ...
                                return (
                                    <div
                                        key={index}
                                        className={
                                            isSelected ? "gridElement selected" : "gridElement"
                                        }
                                        onClick={e => {
                                            e.stopPropagation();
