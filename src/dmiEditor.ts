...
class DmiDocument extends Disposable implements vscode.CustomDocument {
    ...
    private readonly _onWebviewDesynchronized = this._register(
        new vscode.EventEmitter<{ readonly info: string }>()
    );
    ...
    async saveAs(
        ...
        try {
            ...
            if (on_disk_dmi.isSame(this.dmi)) return;
        } catch (error) {
...
export class DmiEditorProvider implements vscode.CustomEditorProvider<DmiDocument> {
        ...
        vscode.commands.registerCommand("dmiEditor.dmiEditor.new", () => {
            ...
            if (!workspaceFolders) {
                vscode.window.showErrorMessage(
                    "Creating new files currently requires opening a workspace"
                );
                return;
            }
            ...
            const uri = vscode.Uri.joinPath(
                ...
                `new-${DmiEditorProvider.newDmiFileId++}.dmi`
    ...
    async openCustomDocument(
        ...
        listeners.push(
            document.onWebviewDesynchronized(e => {
        ...
        switch (message.type) {
            ...
            case MessageType.ReadyRequest:
                return {
                    ...
                    serialized_dmi: document.dmi.serialize(),
                    defaultZoom:
                        vscode.workspace.getConfiguration("dmiEditor").get("defaultZoom") || 2
                };
    ...
    private readonly _onDidChangeCustomDocument = new vscode.EventEmitter<
        vscode.CustomDocumentEditEvent<DmiDocument>
    >();
